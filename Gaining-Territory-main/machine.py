    ...
    def find_best_selection(self):
        ...
        if len(unconnected_points) >= 2:
            ...
            if valid_lines:
                ...
                _, best_line = self.minmax(self.drawn_lines[:], depth=2, alpha=float('-inf'), beta=float('inf'),
                                           maximizing_player=True, tmpscore=tmp_score)
                return best_line
        else:
            _, best_line = self.minmax(self.drawn_lines[:], depth=2, alpha=float('-inf'), beta=float('inf'),
                                       maximizing_player=True, tmpscore=tmp_score)
            ...
            for line1 in connected_lines_point1:
                for line2 in connected_lines_point2:
                    if line1 != line2 and line1 != new_line and line2 != new_line:
                        if self.forms_triangle(new_line, line1, line2) and self.is_valid_triangle(new_line, line1,
                                                                                                  line2):
                            if maximizing_player:
                                current_score[1] += 1
                            else:
